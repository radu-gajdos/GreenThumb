...
const TodayActions: React.FC<TodayActionsProps> = ({ events, onEventClick }) => {
  ...
  // Filtrează evenimentele de azi
  const todayEvents = events.filter(e => isSameDay(e.start, now));
  
  // Separă evenimentele în categorii bazate pe timp
  ...
  const currentEvents = todayEvents.filter(e => {
    ...
    const eventEnd = new Date(e.start.getTime() + (2 * 60 * 60 * 1000)); // Presupunem că o acțiune durează 2 ore
    return eventStart <= now && eventEnd >= now && e.action.status === 'in_progress';
  });
  ...
  // Combină și sortează evenimentele: curente, viitoare, întârziate, completate
  const sortedTodayEvents = [
    ...
    ...completedEvents.sort((a, b) => b.start.getTime() - a.start.getTime()) // Completate în ordine inversă
  ];

  // Funcție pentru a determina status-ul în funcție de timp
  const getTimeBasedStatus = (event: CalendarEvent) => {
    ...
    const eventEnd = new Date(event.start.getTime() + (2 * 60 * 60 * 1000));
    
    if (eventStart <= now && eventEnd >= now) return 'current';
    if (eventStart > now) return 'upcoming';
    if (eventStart < now) return 'overdue';
    
    return event.action.status;
  };

  // Funcție pentru a calcula timpul până la/de la eveniment
  const getTimeRelative = (event: CalendarEvent) => {
    const eventTime = new Date(event.start);
    const diffInMinutes = Math.floor((eventTime.getTime() - now.getTime()) / (1000 * 60));
    
    if (diffInMinutes > 0) {
      if (diffInMinutes < 60) return `în ${diffInMinutes} min`;
      const hours = Math.floor(diffInMinutes / 60);
      return `în ${hours}h ${diffInMinutes % 60}min`;
    } else {
      const pastMinutes = Math.abs(diffInMinutes);
      if (pastMinutes < 60) return `${pastMinutes} min în urmă`;
      const hours = Math.floor(pastMinutes / 60);
      return `${hours}h ${pastMinutes % 60}min în urmă`;
    }
  };
    ...
    return (
      <div className="bg-white rounded-xl border border-gray-200 p-6 h-full flex flex-col">
        <div className="flex items-center mb-4">
          ...
          <h3 className="text-lg font-semibold text-gray-800">Acțiuni de azi</h3>
        </div>
        
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center">
            <Calendar className="w-12 h-12 text-gray-400 mx-auto mb-4" />
            <p className="text-gray-500">Nicio acțiune programată pentru azi</p>
          </div>
        </div>
      </div>
    );
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="space-y-3">
          {sortedTodayEvents.map(event => {
            const timeStatus = getTimeBasedStatus(event);
            ...
            return (
              <div
                ...
                className={`
                  ...
                  ${timeStatus === 'current' ? 'border-blue-300 bg-blue-50' : 
                    timeStatus === 'overdue' ? 'border-red-300 bg-red-50' : 
                    timeStatus === 'completed' ? 'border-green-300 bg-green-50 opacity-75' :
              ...
              >
                  ...
                  <div className="flex items-center space-x-2 mb-1">
                    ...
                    {timeStatus === 'current' && (
                      <div className="flex items-center text-blue-600">
                        ...
                        <span className="text-xs font-medium">ACUM</span>
                      </div>
                    )}
                    {timeStatus === 'overdue' && (
                ...
                <div className="text-right text-sm flex-shrink-0">
                  <p className={`font-medium ${timeStatus === 'current' ? 'text-blue-600' : timeStatus === 'overdue' ? 'text-red-600' : 'text-gray-500'}`}>
